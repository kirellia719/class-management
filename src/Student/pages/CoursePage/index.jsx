...
const AssignmentsPage = () => {
    ...
    const { data: exams, error, isLoading } = useQuery('exams', async () => {
                            ...
                            <TableBody>
                                {exams.map((exam, index) => (
                                    <TableRow key={exam._id}>
                                        ...
                                        <TableCell align='center'>{exam.maxScoreSubmission ? exam.maxScoreSubmission : !exam.attempts ? "Chưa có" : 0}</TableCell>
                                        <TableCell align='center'>{exam.isOpen ? "Đang mở" : "Đã khoá"}</TableCell>
                                        <TableCell align='center'>{<Link to={`/exam/${exam._id}`}><Button >Xem</Button></Link>}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
